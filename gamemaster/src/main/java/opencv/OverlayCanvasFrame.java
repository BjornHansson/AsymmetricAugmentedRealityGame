...
public class OverlayCanvasFrame extends JFrame {
	...
	public OverlayCanvasFrame(String title, GraphicsConfiguration gc, double gamma) {
		super(title, gc);
		init(false, null, gamma);
	}

	public OverlayCanvasFrame(String title, int screenNumber, DisplayMode displayMode) throws Exception {
		this(title, screenNumber, displayMode, 0.0);
	}
	public OverlayCanvasFrame(String title, int screenNumber, DisplayMode displayMode, double gamma) throws Exception {
	...
	private void init(final boolean fullScreen, final DisplayMode displayMode, final double gamma) {
		try {
			if (System.getProperty("os.name").startsWith("Windows")) {
				...
				bombImage = ImageIO.read(new File(classLoader.getResource("bomb_small_sprites.png").getFile()));
				explosionImage = ImageIO.read(new File(classLoader.getResource("explosion_sprites.png").getFile()));
			}
		} catch (IOException e) {
		...
		Runnable r = new Runnable() { public void run() {
			...
			if (fullScreen) {
				...
				setResizable(false);
				gd.setFullScreenWindow(OverlayCanvasFrame.this);
			} else {
	...
	private void drawBombs(Graphics g){
		...
		ArrayList<Bomb> bombs = coloredObjectTrack.getBombs();
	...
	public static OverlayCanvasFrame global = null;
	...
	private KeyEventDispatcher keyEventDispatch = new KeyEventDispatcher() {
		public boolean dispatchKeyEvent(KeyEvent e) {
			if (e.getID() == KeyEvent.KEY_PRESSED) {
				synchronized (OverlayCanvasFrame.this) {
					keyEvent = e;
					OverlayCanvasFrame.this.notify();
				}
			}
			return false;
		}
	...
	public static void tile(final OverlayCanvasFrame[] frames) {
